...
var data2 = ("Rome:Jan 90.2,Feb 73.2,Mar 80.3,Apr 55.7,May 53.0,Jun 36.4,Jul 17.5,Aug 27.5,Sep 60.9,Oct 147.7,Nov 121.0,Dec 97.9\n" +
        "London:Jan 58.0,Feb 38.9,Mar 49.9,Apr 42.2,May 67.3,Jun 52.1,Jul 59.5,Aug 77.2,Sep 55.4,Oct 62.0,Nov 69.0,Dec 52.9\n" +
        "Paris:Jan 182.3,Feb 120.6,Mar 188.1,Apr 204.9,May 323.1,Jun 350.5,Jul 336.8,Aug 192.9,Sep 66.3,Oct 63.3,Nov 83.2,Dec 154.7\n" +
        "NY:Jan 128.7,Feb 121.8,Mar 151.9,Apr 93.5,May 98.8,Jun 93.6,Jul 142.2,Aug 131.8,Sep 92.0,Oct 82.3,Nov 107.8,Dec 94.2\n" +
        "Vancouver:Jan 155.7,Feb 121.4,Mar 132.3,Apr 69.2,May 85.8,Jun 47.1,Jul 31.3,Aug 37.0,Sep 69.6,Oct 116.3,Nov 154.6,Dec 171.5\n" +
...
fun main() {
//    println(mean("London", data)) //51.199999999999996
    println(mean("Lima", data2)) //51.199999999999996
//    println(variance("London", data)) //57.42833333333374

}

fun mean(town: String, strng: String): Double {
    val townDataBeginIndex = strng.indexOf(town) + town.length
    val townDataEndIndex = if(strng.indexOf("\n", townDataBeginIndex) == -1) {
        strng.length
    } else {
        strng.indexOf("\n", townDataBeginIndex)
    }
    val d = strng.substring(townDataBeginIndex + 1, townDataEndIndex)
    return (d.replace("[a-zA-Z]".toRegex(), "").split(",").sumOf { it.toDouble() }) / 12
}
...
fun variance(town: String, strng: String): Double {
    ...
    val d = strng.substring(townDataBeginIndex + 1, townDataEndIndex)
    val list =  d.replace("[a-zA-Z]".toRegex(), "").split(",").map {
        val newValue = it.toDouble() - mean
         newValue * newValue
    }
    return list.sumOf { it } / 12
}
